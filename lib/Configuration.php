...
class Configuration
{
     ...
     * @var string
     */
    protected $host = 'https://www.signwell.com';
     ...
     * @var string
     */
    protected $userAgent = 'SignWell PHP SDK/0.1.0';
    ...
    private static function normalizeHost($host): string
    {
        ...
        $normalized = rtrim(trim($host), '/');
        if ($normalized === '') {
            throw new \InvalidArgumentException('Host must be a non-empty HTTPS URL.');
        }
        ...
        if (
            ...
            throw new \InvalidArgumentException('Host must be a valid HTTPS URL.');
        }

        if (array_key_exists('user', $parts) || array_key_exists('pass', $parts)) {
            throw new \InvalidArgumentException('Host must not include credentials.');
        }

        if (array_key_exists('query', $parts) || array_key_exists('fragment', $parts)) {
            throw new \InvalidArgumentException('Host must not include a query string or fragment.');
        }
    ...
    {
        $report  = 'PHP SDK (SignWell\Sdk) Debug Report:' . PHP_EOL;
